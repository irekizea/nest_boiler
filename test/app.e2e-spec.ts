...
describe('AppController (e2e)', () => {
  ...
  //     const res = await request(app.getHttpServer()).get('/user/emailCheck').send({
  ...
  //     const res = await request(app.getHttpServer()).get('/user/getSellerInfo').
  //     set({Authorization:'Bearer ' + tokenInfo['accessToken']}).send({
  //       userEmail: 'testNew2@test.com',
  //     })
  //     expect(res.statusCode).toBe(401);
  ...
  //     const res = await request(app.getHttpServer()).get('/product/getMarketList')
  //     expect(res.statusCode).toBe(200);
  //   })
  // })
  ...
  //     const res = await request(app.getHttpServer()).post('/product/registProduct').
  //     set({Authorization:'Bearer ' + tokenInfo['accessToken']}).send({
  //       userEmail: 'testNew2@test.com',
  ...
  //       optionList : "[{'optionName':'사이즈', 'optionValue':'M', 'optionQuantity':10}]",
  //       price : 18000,
  ...
  //       deliveryMethod : "{'deliveryOption':'택배 선불', 'deliveryPrice': 1000, 'addOtherProduct': true, 'directLoc': '홍대입구역 2번 출구'}",
  ...
  describe('상품 상세 조회', () => {
    it('/product/getProductDetail (GET)', async () => {
      const res = await request(app.getHttpServer()).get('/product/getProductDetail').send({
        _id: tokenInfo['_id'],
      })
      const productInfo = JSON.parse(res.text)
      expect(res.statusCode).toBe(200);
